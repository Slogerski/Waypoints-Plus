...
final class FightAlertCombinationScreen extends AlertScreen {
    ...
    @Override public boolean keyPressed(KeyEvent input) {
        ...
        if (selected >= 0) {
            if (keyCode == InputConstants.KEY_ESCAPE || keyCode == InputConstants.KEY_DELETE
            ...
            int storedKey = LegacyKeyCodes.fromNative(keyCode);
            if (storedKey >= 0 && !keys.contains(storedKey)) keys.set(selected, storedKey);
