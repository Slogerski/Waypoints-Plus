...
abstract class AlertScreen extends Screen {
    ...
    @Override public final void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float delta) {
        ...
        try {
            extractContent(graphics, (int) (mouseX / renderScale), (int) (mouseY / renderScale), delta);
        } finally {
    ...
    protected boolean clickContent(double x, double y, int button) {
        return super.mouseClicked(mouse(x, y, button), false);
    }
    ...
    @Override public final boolean mouseReleased(MouseButtonEvent event) {
    ...
    static MouseButtonEvent mouse(double x, double y, int button) {
        return new MouseButtonEvent(x, y, new MouseButtonInfo(button, 0));
    }

    static void drawTitle(GuiGraphicsExtractor graphics, net.minecraft.client.gui.Font font,
