...
final class WaypointHudRenderer {
    ...
    private static void renderLabel(Minecraft minecraft, PoseStack pose, SubmitNodeCollector submits,
        ...
        submitLabel(pose, panelSubmits, textSubmits, text, x, textWidth, background, color, textColor);
        pose.popPose();
    }

    private static void submitLabel(PoseStack pose, OrderedSubmitNodeCollector panels,
                                    OrderedSubmitNodeCollector texts, Component text, float x,
                                    int textWidth, int background, int color, int textColor) {
        submitRoundedPanel(pose, panels, x - 3.0f, -7.0f,
    ...
    private static void submitRoundedPanel(PoseStack pose, OrderedSubmitNodeCollector panels,
                                           float left, float top, float right, float bottom,
                                           int background, int border) {
        if ((background >>> 24) != 0) {
            panelRect(pose, panels, left + 2, top + 1, right - 2, top + 2, background);
            panelRect(pose, panels, left + 1, top + 2, right - 1, bottom - 2, background);
            panelRect(pose, panels, left + 2, bottom - 2, right - 2, bottom - 1, background);
        }
        panelRect(pose, panels, left + 2, top, right - 2, top + 1, border);
        panelRect(pose, panels, left + 2, bottom - 1, right - 2, bottom, border);
        panelRect(pose, panels, left, top + 2, left + 1, bottom - 2, border);
        panelRect(pose, panels, right - 1, top + 2, right, bottom - 2, border);
        panelRect(pose, panels, left + 1, top + 1, left + 2, top + 2, border);
        panelRect(pose, panels, right - 2, top + 1, right - 1, top + 2, border);
        panelRect(pose, panels, left + 1, bottom - 2, left + 2, bottom - 1, border);
        panelRect(pose, panels, right - 2, bottom - 2, right - 1, bottom - 1, border);
    }

    private static void panelRect(PoseStack pose, OrderedSubmitNodeCollector panels,
                                  float left, float top, float right, float bottom, int color) {
        panels.submitTextBackground(pose, left, top, right, bottom, color,
                Font.DisplayMode.SEE_THROUGH, FULL_BRIGHT);
    }
