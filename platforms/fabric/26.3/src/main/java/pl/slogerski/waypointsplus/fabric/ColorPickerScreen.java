...
final class ColorPickerScreen extends Screen {
    ...
    @Override public boolean mouseClicked(MouseButtonEvent event, boolean doubled) {
        if (event.button() == 1 && selectHistoryColor(event.x(), event.y())) return true;
        if (event.button() == 1 && beginDrag(event.x(), event.y())) return true;
        return super.mouseClicked(event, doubled);
    }

    @Override public boolean mouseDragged(MouseButtonEvent event, double dx, double dy) {
        if (event.button() == 1 && dragging != 0) { updateSelection(event.x(), event.y()); return true; }
