...
final class FightAlertEditorScreen extends AlertScreen {
    ...
    @Override protected void init() {
        ...
        helpButton = addRenderableWidget(Button.builder(Component.literal("?"), button -> {
            ...
            Blaze3D.openUri(URI.create("https://slogerski.github.io/Waypoints-Plus/?lang="
        ...
        keyButton = button(keyLabel(), 105, y, 267, () -> {
            if ("COMBINATION".equals(triggerType)) {
                minecraft.gui.setScreen(new FightAlertCombinationScreen(this, combinationKeys));
            } else {
                capturingKey = true;
                setFocused(null);
        ...
        if ("MSG".equals(service)) {
            ...
            button(recipients.size() + "/10", 105, y, 267,
                    () -> minecraft.gui.setScreen(new FightAlertRecipientsScreen(this, recipients)));
            y += 28;
    ...
    private void create() {
        ...
        if (saved) minecraft.gui.setScreen(new FightAlertsScreen(parent instanceof FightAlertsScreen list ? list.parentScreen() : parent));
        else {
            refreshCreate();
    ...
    @Override public boolean keyPressed(KeyEvent input) {
        int key = input.key();
        if (capturingKey) {
            if (key != InputConstants.KEY_ESCAPE) keyCode = LegacyKeyCodes.fromNative(key);
            capturingKey = false;
    ...
    static String keyName(int keyCode) {
        return switch (keyCode) {
            case LegacyKeyCodes.LEFT_SHIFT, LegacyKeyCodes.RIGHT_SHIFT -> "Shift";
            case LegacyKeyCodes.LEFT_CONTROL, LegacyKeyCodes.RIGHT_CONTROL -> "Ctrl";
            case LegacyKeyCodes.LEFT_ALT, LegacyKeyCodes.RIGHT_ALT -> "Alt";
            case LegacyKeyCodes.LEFT_SUPER, LegacyKeyCodes.RIGHT_SUPER -> "Super";
            default -> InputConstants.getKey(new KeyEvent(LegacyKeyCodes.toNative(keyCode), 0, 0))
