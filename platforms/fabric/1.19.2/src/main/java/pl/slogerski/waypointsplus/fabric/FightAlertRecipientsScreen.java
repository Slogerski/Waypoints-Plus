...
final class FightAlertRecipientsScreen extends AlertScreen {
    ...
    @Override protected void renderContent(DrawContext context, MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        for (int i = 0; i < count; i++) {
            if (!fields.get(i).visible) continue;
            fields.get(i).render(matrices, mouseX, mouseY, delta);
            ButtonWidget remove = removeButtons.get(i);
            remove.render(matrices, mouseX, mouseY, delta);
            context.drawCenteredTextWithShadow(textRenderer, Text.literal("X"),
                    remove.x + remove.getWidth() / 2 + 1, remove.y + 5, 0xFFFFFFFF);
        }
        context.disableScissor();
        applyButton.render(matrices, mouseX, mouseY, delta);
        AlertScreen.drawTitle(context, textRenderer, title, width / 2.0f, top + 7.0f, matrices);
    }
