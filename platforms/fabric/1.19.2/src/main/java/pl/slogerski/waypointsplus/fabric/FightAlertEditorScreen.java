...
final class FightAlertEditorScreen extends AlertScreen {
    ...
    @Override protected void init() {
        ...
        y += 13;
        messageBox = new EditBoxWidget(textRenderer, left + 12, 0, 360, 88,
                Text.literal(UiText.get("Message", "Wiadomość")), Text.empty());
    ...
    @Override protected boolean scrollContent(double x, double y, double horizontal, double vertical) {
        ...
        if (messageBox.visible && messageBox.isMouseOver(x, y)
                ...
                && messageBox.mouseScrolled(x, y, vertical)) return true;
    ...
    @Override protected void renderContent(DrawContext context, MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        AlertScreen.drawTitle(context, textRenderer, title, left + 195, top + 7, matrices);
        ...
        for (BodyWidget entry : body) {
            ...
            if (entry.outlined()) {
                if (widget instanceof TextFieldWidget)
                    GuiPalette.input(context, widget.x - 5, widget.y - 7, widget.getWidth() + 10, 22);
                else GuiPalette.input(context, widget.x, widget.y, widget.getWidth(), widget.getHeight());
            }
            widget.render(matrices, insideBody(mouseX, mouseY) ? mouseX : -1,
        ...
        for (ButtonWidget widget : footer) widget.render(matrices, mouseX, mouseY, delta);
    }
