...
final class WaypointHudRenderer {
    ...
    private static void render(WorldRenderContext context) {
        ...
        for (Waypoint waypoint : store.waypoints()) {
            ...
            if (settings.laserEnabled) {
                drawLaser(matrices, cameraPos, target,
                        parseArgb(waypoint.colorArgb(), settings.markerArgb));
            }
            renderLabel(client, matrices, camera, cameraPos, waypoint, target, settings);
        }
    }
    ...
    private static void renderLabel(MinecraftClient client, MatrixStack matrices,
        ...
        client.textRenderer.draw(text, x, -4.0f, color, false, matrices.peek().getPositionMatrix(),
                textBuffers, true, 0, FULL_BRIGHT);
        textBuffers.draw();
    ...
    private static void drawRoundedPanel(Matrix4f matrix, float left, float top,
    ...
    private static void drawLaser(MatrixStack matrices, Vec3d cameraPos,
