...
final class WaypointSettingsScreen extends Screen {
    ...
    @Override protected void init() {
        ...
        }).dimensions(left + 10, top + 76, 280, 20).build());
        ...
        addDrawableChild(ButtonWidget.builder(Text.literal(UiText.get("About", "O modzie")),
                ...
                .dimensions(left + 10, top + 184, 280, 20).build());
        addDrawableChild(ButtonWidget.builder(Text.literal(UiText.get("Save", "Zapisz")), b -> save())
                .dimensions(left + 10, top + 208, 136, 20).build());
        addDrawableChild(ButtonWidget.builder(Text.literal(UiText.get("Exit", "Wyjdź")), b -> close())
                .dimensions(left + 154, top + 208, 136, 20).build());
    }
    ...
    private void openProfile(int index, int size, String serverKey) {
        ...
        client.setScreen(new WaypointSettingsScreen(parent, index));
    }
    ...
    private void confirmRemoveProfile(String serverKey) {
        client.setScreen(new ConfirmScreen(confirmed -> {
            ...
            client.setScreen(confirmed ? new WaypointSettingsScreen(parent) : this);
    ...
    @Override public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        if (!virtualProfile) {
            fieldBox(context, left + 10, top + 160, 88, 20);
            fieldBox(context, left + 106, top + 160, 88, 20);
            fieldBox(context, left + 202, top + 160, 88, 20);
        }
        ...
        context.drawTextWithShadow(textRenderer, UiText.get("Scale", "Skala"), left + 10, top + 148, 0xFFD9E2F0);
        context.drawTextWithShadow(textRenderer, "Marker ARGB", left + 106, top + 148, 0xFFD9E2F0);
        context.drawTextWithShadow(textRenderer, UiText.get("Background ARGB", "Tło ARGB"), left + 202, top + 148, 0xFFD9E2F0);
    }
    ...
    private static void gradientOutline(DrawContext context, int left, int top, int right, int bottom) {
        ...
        for (int i = 0; i < segments; i++) {
            ...
            context.fill(x1, bottom - 1, x2, bottom, mix(RED, PURPLE, i / 23.0f));
        }
        ...
        context.fill(left + 1, bottom - 2, left + 2, bottom - 1, RED);
        context.fill(right - 2, bottom - 2, right - 1, bottom - 1, PURPLE);
    }
