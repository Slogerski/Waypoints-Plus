...
final class FightAlertEditorScreen extends AlertScreen {
    ...
    static String keyName(int keyCode) {
        return switch (keyCode) {
            ...
            default -> InputUtil.fromKeyCode(new KeyInput(keyCode, 0, 0)).getLocalizedText().getString();
        };
    }
    ...
    @Override protected boolean clickContent(double x, double y, int button) {
        ...
        if (insideBody(x, y)) {
            for (BodyWidget entry : body) {
                if (entry.widget().mouseClicked(AlertScreen.click(x, y, button), false)) {
            ...
            if (helpButton.mouseClicked(AlertScreen.click(x, y, button), false)) return true;
            for (ButtonWidget widget : footer)
                if (widget.mouseClicked(AlertScreen.click(x, y, button), false)) return true;
        }
        setFocused(null);
        return false;
    }
    ...
    @Override protected void renderContent(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        AlertScreen.drawTitle(context, textRenderer, title, left + 195, top + 7);
