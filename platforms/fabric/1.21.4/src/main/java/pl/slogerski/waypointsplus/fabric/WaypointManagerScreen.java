...
final class WaypointManagerScreen extends Screen {
    ...
    @Override protected void init() {
        ...
        addDrawableChild(ButtonWidget.builder(Text.empty(), b -> toggleAll(allSelected))
                .dimensions(left - 1, 48, 20, 20).build()).active = !entries.isEmpty();
        ...
        for (int i = from; i < Math.min(entries.size(), from + PAGE_SIZE); i++) {
            ...
            addDrawableChild(ButtonWidget.builder(Text.empty(),
                    b -> toggle(waypoint)).dimensions(left - 1, rowY, 20, 20).build());
    ...
    @Override public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        context.drawTextWithShadow(textRenderer, WaypointCountText.format(entries.size()), left + 26, 54, 0xFF555555);
        ...
        for (int i = from; i < Math.min(entries.size(), from + PAGE_SIZE); i++) {
            ...
            if (w.id().equals(selection.confirmSingleDelete)) {
                context.drawTextWithShadow(textRenderer, UiText.get("Sure?", "Na pewno?"), left + 365, y, 0xFFFFA0A0);
            }
            String line = WaypointsPlusClient.config().settings().showCoordinates
                    ? w.name() + "  [" + w.x() + ", " + w.y() + ", " + w.z() + "]"
                    : w.name();
            context.drawTextWithShadow(textRenderer, line, left + 26, y, parseColor(w.colorArgb()));
        }
    }
