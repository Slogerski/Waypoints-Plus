...
final class WaypointSettingsScreen extends Screen {
    ...
    private WaypointSettingsScreen(Screen parent, int profileIndex) {
    ...
    @Override protected void init() {
        ...
        if (virtualProfile) {
            addRenderableWidget(Button.builder(Component.literal(UiText.get("Add profile", "Dodaj profil")),
                    b -> minecraft.setScreen(new ProfileNameScreen(this, parent)))
        ...
        Button edit = addRenderableWidget(Button.builder(Component.literal(UiText.get("Edit", "Edytuj")),
                b -> minecraft.setScreen(new ProfileNameScreen(this, parent, profileName)))
        ...
        addRenderableWidget(Button.builder(Component.literal(UiText.get("Language: English", "Język: Polski")), b -> {
            ...
            minecraft.setScreen(new WaypointSettingsScreen(parent));
        }).pos(left + 10, top + 76).size(280, 20).build());
        addToggle(left + 10, top + 100, 136, UiText.get("Background", "Tło"), settings.background, v -> settings.background = v);
        addToggle(left + 154, top + 100, 136, UiText.get("Coordinates", "Koordynaty"), settings.showCoordinates, v -> settings.showCoordinates = v);
        addToggle(left + 10, top + 124, 136, UiText.get("Distance", "Odległość"), settings.showDistance, v -> settings.showDistance = v);
        addToggle(left + 154, top + 124, 136, UiText.get("Laser", "Laser"), settings.laserEnabled, v -> settings.laserEnabled = v);

        scale = field(left + 10, top + 160, 88, String.valueOf(settings.scale), "Scale");
        markerColor = field(left + 106, top + 160, 88, String.format("%08X", settings.markerArgb), "Marker");
        backgroundColor = field(left + 202, top + 160, 88, String.format("%08X", settings.backgroundArgb), "Background");
        addRenderableWidget(Button.builder(Component.literal(UiText.get("About", "O modzie")),
                b -> minecraft.setScreen(new AboutScreen(this)))
                .pos(left + 10, top + 184).size(280, 20).build());
        addRenderableWidget(Button.builder(Component.literal(UiText.get("Save", "Zapisz")), b -> save())
                .pos(left + 10, top + 208).size(136, 20).build());
        addRenderableWidget(Button.builder(Component.literal(UiText.get("Exit", "Wyjdź")), b -> onClose())
                .pos(left + 154, top + 208).size(136, 20).build());
    }
    ...
    private void openProfile(int index, int size, String serverKey) {
        ...
        minecraft.setScreen(new WaypointSettingsScreen(parent, index));
    }
    ...
    private void confirmRemoveProfile(String serverKey) {
        minecraft.setScreen(new ConfirmScreen(confirmed -> {
            ...
            minecraft.setScreen(confirmed ? new WaypointSettingsScreen(parent) : this);
    ...
    @Override public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float delta) {
        ...
        int bottom = virtualProfile ? top + 150 : top + 230;
        roundedFill(graphics, left, top, left + 300, bottom, 0xE0141824);
        gradientOutline(graphics, left, top, left + 300, bottom);
        if (!virtualProfile) {
            fieldBox(graphics, left + 10, top + 160, 88, 20);
            fieldBox(graphics, left + 106, top + 160, 88, 20);
            fieldBox(graphics, left + 202, top + 160, 88, 20);
        }
        ...
        graphics.text(font, UiText.get("Scale", "Skala"), left + 10, top + 148, 0xFFD9E2F0, true);
        graphics.text(font, "Marker ARGB", left + 106, top + 148, 0xFFD9E2F0, true);
        graphics.text(font, UiText.get("Background ARGB", "Tło ARGB"), left + 202, top + 148, 0xFFD9E2F0, true);
    }
    ...
    private static void gradientOutline(GuiGraphicsExtractor graphics, int left, int top, int right, int bottom) {
        ...
        for (int i = 0; i < segments; i++) {
            ...
            graphics.fill(x1, bottom - 1, x2, bottom, mix(RED, PURPLE, i / 23.0f));
        }
        ...
        graphics.fill(left + 1, bottom - 2, left + 2, bottom - 1, RED);
        graphics.fill(right - 2, bottom - 2, right - 1, bottom - 1, PURPLE);
    }
